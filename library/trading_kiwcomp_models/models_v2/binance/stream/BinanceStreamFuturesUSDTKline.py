...
class BinanceStreamFuturesUSDTKline(BaseModel):
    source: str = "binance"
    type: str = "kline"
...
class BinanceStreamFuturesUSDTKlineTable(StreamKline):
    __tablename__ = "binance_stream_futures_usdt_kline"
...
# ✅ Mapper
class BinanceStreamFuturesUSDTKlineMapper:
    @staticmethod
    def from_raw(raw: dict) -> BinanceStreamFuturesUSDTKline:
        d = raw.get("data") or raw
        k = d.get("k") or {}

        return BinanceStreamFuturesUSDTKline(
            symbol=d["s"],
            interval=k["i"],
    ...
    @staticmethod
    def to_table(event: BinanceStreamFuturesUSDTKline) -> BinanceStreamFuturesUSDTKlineTable:
        return BinanceStreamFuturesUSDTKlineTable(
            symbol=event.symbol,
